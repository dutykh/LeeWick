...
MatrixAssembler := proc (
  ...
  sol := fsolve({f(xe,alpha) = 0, diff(f(xe,alpha),xe) = 0}, {xe = 0.1..1.0, alpha = 5.0..5.5}):
  assign(sol); # we assign the values of critical parameters
  tol := 1e-4:
  known_xe := 0.62987:
  known_alpha := 5.18635:
  if abs(xe - known_xe) > tol or abs(alpha - known_alpha) > tol then
   error(1, "Calculated values are too far from expected values!");
  end if:
  # Determination of the event horizon:
  h := x -> 1 - exp(-alpha*x)*((1 + alpha/2*(1 + q^2)*x)*cos(alpha*q*x) + 1/(2*q)*(1 - q^2 + alpha*(1 + q^2)*x)*sin(alpha*q*x)):
  xh := fsolve(x - h(x) = 0, x = 1.0..1.5):
  known_xh := 1.040133223:
  if abs(xh - known_xh) > tol then
   error(1, "Calculated values are too far from expected values!");
  end if:
